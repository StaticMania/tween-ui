...
export interface HeroTabWipeSlide {
...
export interface HeroTabWipeProps extends ComponentPropsWithoutRef<'section'> {
  /** Slides in the stage. The tab strip mirrors this list. */
  slides?: HeroTabWipeSlide[];
}

const SHOT = (id: string) => `https://images.unsplash.com/${id}?w=1200&h=750&fit=crop`;

const DEFAULT_SLIDES: HeroTabWipeSlide[] = [
...
export default function HeroTabWipe({
  ...
  useGSAP(
    () => {
      ...
      const ease =
        CustomEase.get('hero-tab-wipe') || CustomEase.create('hero-tab-wipe', '0.625, 0.05, 0, 1');
