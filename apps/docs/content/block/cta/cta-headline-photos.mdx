...
that drops into place. The action is a [Slide Arrow Button](/component/button/slide-arrow-button).
GSAP-driven, and it fully respects `prefers-reduced-motion`.

::component-preview{name="cta-headline-photos" size="xl"}
::
...
::install-tabs{item="@tweenui/cta-headline-photos"}
::

:::
:::tabs-item{label="Manual" icon="i-lucide-clipboard"}

Install [Slide Arrow Button](/component/button/slide-arrow-button) first, then
copy the block:

::component-source{name="cta-headline-photos"}
::
...
```tsx [app/page.tsx]
import CtaHeadlinePhotos from '@/components/tweenui/cta/cta-headline-photos';
