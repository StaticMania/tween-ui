...
describe('Testimonial Split Slide', () => {
  it('renders every quote and image', () => {
    render(<TestimonialSplitSlide testimonials={TESTIMONIALS} />);
  ...
  it('marks the first slide active', () => {
    const { container } = render(<TestimonialSplitSlide testimonials={TESTIMONIALS} />);
    expect(container.querySelector('[data-testimonial-split-slide]')).toHaveAttribute(
      'data-active-slide',
      '0'
    );
  });
  ...
  it('wipes left-to-right on next and right-to-left on prev', () => {
    const { container } = render(<TestimonialSplitSlide testimonials={TESTIMONIALS} />);
    const root = () => container.querySelector('[data-testimonial-split-slide]');
  ...
  it('keeps the original right-to-left split when directional is false', () => {
    const { container } = render(
      <TestimonialSplitSlide testimonials={TESTIMONIALS} directional={false} />
    );
  ...
  it('merges a custom className onto the section', () => {
    const { container } = render(
      <TestimonialSplitSlide testimonials={TESTIMONIALS} className="mt-10" />
    );
    expect(container.querySelector('[data-testimonial-split-slide]')).toHaveClass('mt-10');
  });
});
