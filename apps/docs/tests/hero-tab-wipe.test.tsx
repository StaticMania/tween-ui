...
import HeroTabWipe, { type HeroTabWipeSlide } from '@/registry/tweenui/hero-tab-wipe';

const SLIDES: HeroTabWipeSlide[] = [
  { image: '/a.jpg', alt: 'Workflow board' },
  { image: '/b.jpg', alt: 'Performance dashboard', tabImage: '/b-thumb.jpg' },
];

describe('Hero Tab Wipe', () => {
  it('renders one stage slide per item', () => {
    const { container } = render(<HeroTabWipe slides={SLIDES} />);

    expect(container.querySelectorAll('[data-hero-tab-wipe-slide]')).toHaveLength(2);
  ...
  it('builds one tab per slide, with the first selected', () => {
    render(<HeroTabWipe slides={SLIDES} />);
  ...
  it('merges a custom className onto the section', () => {
    const { container } = render(<HeroTabWipe slides={SLIDES} className="mt-10" />);
    expect(container.querySelector('[data-hero-tab-wipe]')).toHaveClass('mt-10');
  });
});
